...
import { TERMS_TEXT } from "@/lib/tree/copy";
import { LegalSection } from "@/components/legal/LegalSection";

export const metadata: Metadata = {
  title: "Условия использования — Дерево Опоры",
  description: TERMS_TEXT.tagline,
};
  ...
  return (
    <div className="min-h-full bg-cream">
      ...
      <article className="mx-auto max-w-[680px] p-[48px_32px_80px]">
        <p className="mb-2 font-serif text-sm text-ink-muted italic">{TERMS_TEXT.tagline}</p>
        <h1 className="mb-2 font-serif text-[36px] leading-[1.15] font-medium tracking-[-0.02em] text-ink">
          {TERMS_TEXT.title}
        </h1>
        <p className="mb-8 text-xs text-ink-muted">{TERMS_TEXT.lastUpdated}</p>
        <p className="mb-10 text-[15px] leading-[1.75] text-ink-soft">{TERMS_TEXT.intro}</p>

        {TERMS_TEXT.sections.map((section) => (
